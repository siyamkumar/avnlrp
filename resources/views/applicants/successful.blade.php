<x-guest-layout>

    <x-slot name="header">
        <div class="text-center">
            <svg viewBox="0 0 1024 1024" class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#000000"
                width="50">
                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                <g id="SVGRepo_iconCarrier">
                    <path
                        d="M905.92 237.76a32 32 0 0 0-52.48 36.48A416 416 0 1 1 96 512a418.56 418.56 0 0 1 297.28-398.72 32 32 0 1 0-18.24-61.44A480 480 0 1 0 992 512a477.12 477.12 0 0 0-86.08-274.24z"
                        fill="#198754"></path>
                    <path
                        d="M630.72 113.28A413.76 413.76 0 0 1 768 185.28a32 32 0 0 0 39.68-50.24 476.8 476.8 0 0 0-160-83.2 32 32 0 0 0-18.24 61.44zM489.28 86.72a36.8 36.8 0 0 0 10.56 6.72 30.08 30.08 0 0 0 24.32 0 37.12 37.12 0 0 0 10.56-6.72A32 32 0 0 0 544 64a33.6 33.6 0 0 0-9.28-22.72A32 32 0 0 0 505.6 32a20.8 20.8 0 0 0-5.76 1.92 23.68 23.68 0 0 0-5.76 2.88l-4.8 3.84a32 32 0 0 0-6.72 10.56A32 32 0 0 0 480 64a32 32 0 0 0 2.56 12.16 37.12 37.12 0 0 0 6.72 10.56zM230.08 467.84a36.48 36.48 0 0 0 0 51.84L413.12 704a36.48 36.48 0 0 0 51.84 0l328.96-330.56A36.48 36.48 0 0 0 742.08 320l-303.36 303.36-156.8-155.52a36.8 36.8 0 0 0-51.84 0z"
                        fill="#198754"></path>
                </g>
            </svg>
            <h2 class="">Success</h2>
        </div>
    </x-slot>

    <div class="container">
        <div class="row">
            <div class="col-lg-4 col-md-3 cold-sm-12 mx-auto">
                <x-card class="border-0 rounded-0 p-3 text-center">

                    You have shown interest in the{{ $arn->jobpostings->jobTitle }}. To complete the submission of
                    application please continue and fill the required details. Your application referenece Number is

                    <h3 class="mt-2"># {{ $arn->arn }}</h3>

                    <a href="{{ route('jobapplication.index') }}" class="btn btn-primary ">Continue

                        <svg viewBox="-3 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" width="36"
                            xmlns:xlink="http://www.w3.org/1999/xlink" fill="#ffffff">
                            <g>
                                <path
                                    d="M13.906 21.637l0.742 0.742 6.378-6.379-6.378-6.379-0.742 0.742 5.112 5.112h-12.727v1.049h12.727z"
                                    fill="#FFFFFF"> </path>
                            </g>
                        </svg>

                    </a>
                </x-card>
            </div>
        </div>
    </div>


    {{-- {{ $arn }} --}}
</x-guest-layout>
